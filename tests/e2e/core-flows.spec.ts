...
test("project editor can save a canvas edit and export a PNG", async ({ page, request }) => {
  ...
  try {
    await page.goto(`/projects/${projectId}/editor`, { waitUntil: "domcontentloaded" });
    await expect(page.getByRole("heading", { name: projectName })).toBeVisible();
    await expect(page.getByRole("button", { name: "Salva progetto" })).toBeVisible();
    await expect(page.locator('.editor-shell[data-hydrated="true"]')).toBeVisible({ timeout: 45_000 });
    await expect(page.getByText("Pronto")).toBeVisible();

    await page.locator(".editor-tool-grid").getByRole("button", { exact: true, name: "Cancella" }).click();
    ...
    expect(savedBody.project.document.tiles.find((tile) => tile.x === 3 && tile.y === 3)?.kind).toBe("empty");
...
test("project snapshots can diff and restore document changes", async ({ request }) => {
  const project = await createPlaywrightProject(request, `Playwright Snapshot ${Date.now()}`);

  try {
    const projectResponse = await request.get(`/api/projects/${project.id}`);
    expect(projectResponse.status()).toBe(200);
    const projectBody = (await projectResponse.json()) as { project: TestProject };
    const originalDocument = projectBody.project.document;
    const originalTile = originalDocument.tiles.find((tile) => tile.kind !== "empty") ?? originalDocument.tiles[0];
    ...
    const changedTile = changedDocument.tiles.find((tile) => tile.x === originalTile.x && tile.y === originalTile.y);
    ...
    const restoreResponse = await request.post(`/api/projects/${project.id}/snapshots/${snapshotBody.snapshot.contentHash}`);
    expect(restoreResponse.status()).toBe(200);

    const restoredResponse = await request.get(`/api/projects/${project.id}`);
    expect(restoredResponse.status()).toBe(200);
    const restoredBody = (await restoredResponse.json()) as { project: TestProject };
...
test("project export API returns WEBP dd2vtt and Session Pack artifacts", async ({ request }) => {
  const project = await createPlaywrightProject(request, `Playwright Export ${Date.now()}`, {
    heightCells: 8,
    roomCount: 2,
    widthCells: 10
  });
  ...
  try {
    const webpResponse = await request.post(`/api/projects/${project.id}/export`, {
      data: {
        format: "webp",
        includeGrid: false,
        scale: 1
    ...
    const dd2vttResponse = await request.post(`/api/projects/${project.id}/export`, {
      data: {
        format: "dd2vtt",
        includeGrid: false,
        scale: 1
    ...
    expect(dd2vttResponse.headers()["content-type"]).toContain("application/json");
    ...
    const sessionPackResponse = await request.post(`/api/projects/${project.id}/export`, {
      data: {
    ...
    expect(sessionPackResponse.headers()["content-type"]).toContain("application/zip");
...
async function deleteProjectQuietly(request: APIRequestContext, projectId: string): Promise<void> {
  await request.delete(`/api/projects/${projectId}`).catch(() => {
    // Cleanup is best-effort: project data lives in ignored local runtime folders.
  });
}
