...
const fixturesRoot = path.resolve(
  fileURLToPath(new URL("../src/migrations/fixtures", import.meta.url))
);
...
describe("migrateMapDocument", () => {
  it("accepts current v1 documents", async () => {
    const document = migrateMapDocument(
      await readFixture("document-v1-basic.json")
    );
  ...
  it("migrates legacy documents without an explicit version", async () => {
    const document = migrateMapDocument(
      await readFixture("document-v0-basic.json")
    );
  ...
  it("preserves assets and plan data from v1 fixtures", async () => {
    const withAssets = migrateMapDocument(
      await readFixture("document-v1-with-assets.json")
    );
    const withPlan = migrateMapDocument(
      await readFixture("document-v1-with-plan.json")
    );
  ...
  it("fails clearly for unsupported future versions", () => {
    expect(() => migrateMapDocument({ version: 99 })).toThrow(
      MapDocumentMigrationError
    );
    expect(() => migrateMapDocument({ version: 99 })).toThrow(
      "Unsupported MapDocument version: 99"
    );
  });

  it("upgrades v1 documents to the v2 preparation schema", async () => {
    const document = upgradeMapDocumentToV2(
      await readFixture("document-v1-basic.json")
    );
  ...
  it("accepts v2 documents through the explicit v2 upgrader", async () => {
    const document = upgradeMapDocumentToV2(
      await readFixture("document-v2-basic.json")
    );
  ...
  it("fails clearly for corrupt documents", () => {
    expect(() => migrateMapDocument({ id: "", version: 1 })).toThrow(
      MapDocumentMigrationError
    );
    expect(() => migrateMapDocument({ id: "", version: 1 })).toThrow(
      "Invalid MapDocument after migration"
    );
  });
});
