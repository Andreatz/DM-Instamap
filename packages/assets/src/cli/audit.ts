...
async function main(): Promise<void> {
  const audit = await auditAssets({ outputRoot: process.env.INIT_CWD ?? process.cwd() });
...
main().catch((error: unknown) => {
  process.stderr.write(`${error instanceof Error ? error.message : "Unknown asset audit error."}\n`);
  process.exitCode = 1;
});
