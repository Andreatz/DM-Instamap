...
const SPECIAL_TAG =
  /boss|final|treasure|tesoro|altar|chapel|cappella|library|biblioteca|forge|shrine|water|river|fiume|reliqu/u;
...
export const BENCHMARK_SCENARIOS: BenchmarkScenario[] = [
  {
    ...
    styleDna: {
      densityBias: "rich",
      layoutBias: "compact",
      paletteTags: ["stone", "candlelight", "bone"]
    },
    theme: "crypt",
    thresholds: {
      minScore: 90,
  ...
  },
  {
    build: () =>
      generateMapFromBlueprint(
        generateBuildingBlueprint({
          request: "a lively tavern with common room, kitchen and cellar",
          theme: "tavern"
        })
      ),
    id: "tavern",
...
export function computeBenchmarkMetrics(
  ...
  return {
    ...
    readability: blend(
      quality.metrics.walkableBalance.score,
      0.6,
      quality.metrics.roomUtility.score,
      0.4
    ),
    roomVariety: scoreRoomVariety(rooms),
    routing: blend(
      quality.metrics.connectivity.score,
      0.7,
      quality.metrics.deadEnds.score,
      0.3
    ),
    tacticalAffordance: blend(
      quality.metrics.tacticalCover.score,
      0.6,
      quality.metrics.lineOfSightBreaks.score,
      0.4
    ),
...
function collectFailures(
  ...
  if (quality.score < thresholds.minScore) {
    failures.push(
      `punteggio ${quality.score} sotto la soglia ${thresholds.minScore}`
    );
  }
  ...
  for (const key of Object.keys(METRIC_LABELS) as BenchmarkMetricKey[]) {
    ...
    if (minimum !== undefined && metrics[key] < minimum) {
      failures.push(
        `${METRIC_LABELS[key]} ${metrics[key]} sotto la soglia ${minimum}`
      );
    }
  }
...
function scoreRoomVariety(rooms: RoomNode[]): number {
  ...
  const distinctLabels = new Set(rooms.map((room) => room.label.toLowerCase()))
    .size;
...
function scoreAssetDensity(document: MapDocument, roomCount: number): number {
  ...
  const specialRooms = (document.plan?.rooms ?? []).filter((room) =>
    room.tags.some((tag) => SPECIAL_TAG.test(tag))
  ).length;
  const featureCount =
    doors + lights + gmNotes + placements + placedAssets + specialRooms;
