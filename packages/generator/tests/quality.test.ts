...
describe("scoreMapQuality", () => {
  ...
  it("warns about disconnected walkable regions", () => {
    ...
    expect(
      quality.debugTiles.some((tile) => tile.kind === "disconnected")
    ).toBe(true);
  ...
  it("reports dead ends for thin one-way corridors", () => {
    ...
    expect(quality.debugTiles.some((tile) => tile.kind === "dead-end")).toBe(
      true
    );
  });
});
