...
describe("dd2vtt grid/image alignment", () => {
  it("renders the embedded image at exactly pixels_per_grid per cell", async () => {
    ...
    expect(meta.width).toBe(
      result.object.resolution.map_size.x *
        result.object.resolution.pixels_per_grid
    );
  });
  ...
  it("keeps logical resolution when the image is not embedded", async () => {
    const result = await exportMapDocumentDd2Vtt(createRealisticMap(), {
      embedImage: false
    });
  ...
  it("preserves walls, doors and lights through an export/import round-trip", async () => {
    const exported = await exportMapDocumentDd2Vtt(createRealisticMap(), {
      embedImage: false
    });
    ...
    expect(
      imported.document.plan?.doors.find((door) => door.isOpen)
    ).toBeDefined();
  });
});
...
describe("foundry version compatibility", () => {
  it("defaults to verified v12 compatibility", () => {
    const data = buildFoundryModuleData(createRealisticMap(), {
      moduleId: "compat-default"
    });
    expect(data.moduleJson.compatibility).toEqual({
      minimum: "11",
      verified: "12"
    });
  });
  ...
  it("scales scene dimensions and grid size with the document grid", () => {
    ...
    expect(data.sceneJson.walls.filter((wall) => wall.door === 1)).toHaveLength(
      2
    );
    expect(
      data.sceneJson.walls.find(
        (wall) => wall._id === data.sceneJson.walls[0]?._id
      )?.c
    ).toHaveLength(4);
  });
});
