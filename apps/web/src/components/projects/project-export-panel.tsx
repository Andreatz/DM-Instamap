...
const FORMAT_OPTIONS: Array<{
  ...
  {
    description: "Immagine raster per schermo e stampa.",
    extension: "png",
    label: "PNG",
    value: "png"
  },
  {
    description: "Immagine compressa per il web.",
    extension: "webp",
    label: "WEBP",
    value: "webp"
  },
  {
    description: "Universal VTT (import in Foundry, Roll20).",
    extension: "dd2vtt",
    label: "dd2vtt",
    value: "dd2vtt"
  },
  {
    description: "Modulo Foundry installabile (ZIP).",
    extension: "zip",
    label: "Modulo Foundry",
    value: "foundry"
  },
  {
    description: "JSON modificabile proprietario.",
    extension: "dmimap.json",
    label: "dmimap",
    value: "dmimap"
  },
  {
    description:
      "ZIP completo: mappe full / GM / giocatore, note GM, iniziativa, manifest.",
    extension: "zip",
    label: "Session Pack",
    value: "session-pack"
  }
...
const MODE_OPTIONS: Array<{
  ...
  {
    description: "Nasconde stanze segrete, trappole, note GM e annotazioni.",
    label: "Sicuro per i giocatori",
    value: "player"
  },
  {
    description: "Tutto visibile (per il riferimento personale del DM).",
    label: "Game Master",
    value: "gm"
  },
  {
    description: "Come la modalita giocatore, ma rimuove anche note e luci.",
    label: "Pulito",
    value: "clean"
  }
];
  ...
  async function exportProject() {
    ...
    try {
      ...
      const response = await fetch(endpoint, {
        body: JSON.stringify({
          description:
            format === "session-pack" ? sessionDescription : undefined,
          document: projectId ? undefined : document,
          format,
          includeGrid,
          includeInitiative:
            format === "session-pack" ? includeInitiative : undefined,
      ...
      const extension =
        supportsSplitLayers && splitLayers ? "zip" : formatExtension(format);
      link.href = url;
        ...
        <>
          ...
          {supportsSplitLayers ? (
            <label className="editor-checkbox">
              <input
                checked={splitLayers}
                onChange={(event) => setSplitLayers(event.target.checked)}
                type="checkbox"
              />
