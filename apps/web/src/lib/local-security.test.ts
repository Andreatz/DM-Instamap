...
import { isLocalHostHeader, isRemoteAccessAllowed, shouldBlockRemoteRequest } from "./local-security";
...
describe("local-security", () => {
  ...
  it("blocks remote host headers unless explicitly allowed", () => {
    ...
    expect(shouldBlockRemoteRequest("dm-instamap.example.com", { DM_INSTAMAP_ALLOW_REMOTE: "true" })).toBe(false);
  });

  it("requires the exact true flag for remote access", () => {
    expect(isRemoteAccessAllowed({ DM_INSTAMAP_ALLOW_REMOTE: "1" })).toBe(false);
    expect(isRemoteAccessAllowed({ DM_INSTAMAP_ALLOW_REMOTE: "true" })).toBe(true);
  });
});
