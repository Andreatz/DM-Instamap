...
export async function GET(_request: Request, context: PreviewRouteContext) {
  ...
  const reference = manifest.references.find((candidate) => candidate.id === referenceId);
  ...
  const previewPath = path.resolve(workspaceRoot, "data", "previews", "references", `${reference.id}.webp`);
