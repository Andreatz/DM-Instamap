import {
  ...
  PACK_PRESETS,
  type PackPreset
} from "@dm-instamap/assets/pack-importer";
...
export async function POST(request: Request) {
  try {
    ...
    const assetRoot =
      typeof body.assetRoot === "string" ? body.assetRoot.trim() : "";
    const preset = typeof body.preset === "string" ? body.preset.trim() : "";

    if (!assetRoot) {
      return Response.json(
        { error: "assetRoot is required.", ok: false },
        { status: 400 }
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : "Could not import asset pack.";
    return Response.json(
      { error: message, ok: false },
      { status: error instanceof LocalPathValidationError ? 400 : 500 }
    );
  }
}
