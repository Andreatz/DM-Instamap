import { createProviderFromEnv, describeMapWithAi } from "@dm-instamap/ai-bridge";
...
export async function POST(request: Request) {
  try {
    ...
    if (!mapName.trim()) {
      return Response.json({ error: "mapName is required.", ok: false }, { status: 400 });
    }

    const rooms = Array.isArray(body.rooms)
      ? body.rooms.filter((room): room is { id: string; label: string; tags?: string[] } => {
    ...
    const result = await describeMapWithAi({ mapName, rooms, styleTags, theme }, provider);
